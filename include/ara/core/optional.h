 ...
 *
 * @req {SWS_CORE_01030}
 * value member function overloads, no member functions with the name exists in
 * ara::core::Optional.
 *
 * @req {SWS_CORE_01031}
...
template<typename T> class Optional
{
 public:
     ...
     * a moved-from optional still contains a value, but the value itself is
     * moved from. If other does not contain a value, constructs an object that
     ...
     * @brief Constructs an optional object that contains a value,
     * initialized as if direct-initializing (but not direct-list-initializing)
     * an object of type T from the arguments ilist,
     * std::forward<Args>(args)....
     *
     * @tparam U initializer list type.
     * @tparam Args args type.
     *
     * @param ilist initializer list with which to initialize the contained
     * value.
     * @param args arguments with which to initialize the contained value.
     */
    ...
    constexpr explicit Optional(in_place_t,
                                ...
                                Args&&... args)
      : o_(std::in_place, ilist, std::forward<Args>(args)...)
    {}
     ...
     */
    template<class U> explicit Optional(const Optional<U>& other) : o_(other.o_)
    {}
     ...
     * @return reference to this.
     */
    Optional& operator=(const Optional& other) { o_ = other.o_; }
     ...
     */
    void swap(Optional& other) noexcept { o_.swap(other.o_); }
     ...
     */
    constexpr const T* operator->() const { return &o_.value(); }
     ...
     * @return pointer to the contained value.
     */
    constexpr T* operator->() { return &o_.value(); }
     ...
     * @return reference to the contained value.
     */
    constexpr T&& operator*() && { return std::move(*o_.value()); }
     ...
     * @return reference to the contained value.
     */
    constexpr const T&& operator*() const&& { return std::move(*o_.value()); }

    /**
     * @brief Checks whether *this contains a value.
     *
     * @return true if *this contains a value, false if *this does not contain a
     * value.
     */
     ...
     *
     * @return The current value if *this has a value, or default_value
     * otherwise.
     */
 ...
 * Otherwise, if bool(lhs) == false (and so bool(rhs) == false as well), returns
 * true. Otherwise, returns *lhs == *rhs.
 */
 ...
 * Otherwise, if bool(lhs) == false (and so bool(rhs) == false as well), returns
 * false. Otherwise, returns *lhs != *rhs.
 */
template<class T, class U> constexpr bool
operator!=(const Optional<T>& lhs, const Optional<U>& rhs)
{
...
operator==(const Optional<T>& opt, nullopt_t) noexcept
{
    return ! bool(opt);
}
 ...
 */
template<class T> constexpr bool
operator==(nullopt_t, const Optional<T>& opt) noexcept
{
    return ! bool(opt);
}
 ...
 */
template<class T> constexpr Optional<std::decay_t<T>> make_optional(T&& value)
{
 ...
 * @return the constructed optional object.
 */
template<class T, class... Args> constexpr Optional<T>
